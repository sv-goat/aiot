...
def handle_request(conn):
    ...
    try:
        ...
        if func:
            print(f"Executing {func_name} with args {data.get('args', [])}")
            args = data.get("args", [])
            ...
            # [FIXED] Removed the special case for asyncio.run().
            # All functions are now called synchronously.
...
rtc = RTC()
rtc.datetime((2025, 9, 26, 4, 6, 7, 0, 0))
...
def get_har_data():
    """
    Capture 128 samples of 3-axis accelerometer data and return a JSON-serializable dict.
    """
    gc.collect()
    # Notify user on OLED
    display_multiline_text("Collecting", "sensor data...")

    acc_x, acc_y, acc_z = [], [], []
    print("Starting HAR data capture (128 samples at ~50Hz)...")

    for _ in range(128):
        try:
            x, y, z = read_acceleration()
        except Exception as e:
            # If a single read fails, append zeros and continue
            print("Warning: read_acceleration failed:", e)
            x, y, z = 0.0, 0.0, 0.0

        acc_x.append(float(x))
        acc_y.append(float(y))
        acc_z.append(float(z))
        # MicroPython: sleep_ms is available on ESP32
        try:
            time.sleep_ms(20)
        except AttributeError:
            # Fallback for environments without sleep_ms
            time.sleep(0.02)

    print("Capture complete. Collected {} samples.".format(len(acc_x)))

    # Return a dict so the host can easily consume the data via JSON
    sensor_data = {
        "acc_x": acc_x,
        "acc_y": acc_y,
        "acc_z": acc_z,
...
def vm_har_predict():
    '''
    Capture HAR data and send it to the VM endpoint for prediction. Then, display the result.
    '''
    ...
    vm_url = "http://35.229.130.144:5000/predict_har"
    try:
        response = requests.post(vm_url, json={"data": sensor_array}, timeout=35)
        result = response.json()
        activity = result.get("activity", "Unknown")
        display_multiline_text("Activity:", activity)
        return f"Predicted activity: {activity}"
    except Exception as e:
        print("VM HAR prediction failed:", e)
